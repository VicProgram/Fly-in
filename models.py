...
#             raise ValueError(
#                 f"Error: ya existe un hub en la coordenada ({hub.x}, {hub.y})."
#                 )
...
#             Drone(
#                 f"D{i}",
#                 self.map.start_hub) for i in range(1, drones_number + 1)
#         ]
...
#                 {self.get_drones_in_hub(self.map.start_hub)}"
#             )
...
class Drone:
    ...
    def get_drone_info(self) -> None:
        # Corregido: Ahora muestra las variables reales correspondientes
        print(f"Drone_id: {self.id}")
        print(f"Drone location: {self.location.name if self.location else 'None'}")
...
class Solver:
    def __init__(self, drone_map: Drone_Map, drones_number: int) -> None:
        ...
        self.drones: list[Drone] = [
            Drone(f"D{i}", self.map.start_hub) for i in range(1, drones_number + 1)
        ]
